...
<div class="vp-hero">
    ...
    <p class="vp-hero-sub">{APP_TAGLINE}. Load ETAS INCA MF4/DAT files, average
...
st.set_page_config(
    ...
    menu_items={
        "About": (
            f"### {APP_TITLE}\n"
            "Analyze ETAS INCA MF4/DAT measurement files, average repeated runs, and "
        ...
        if not measurement_folder.is_dir():
            ...
            local_files = sorted(
                [
                    ...
                    if path.is_file() and path.suffix.casefold() in {".mf4", ".dat"}
                ],
                key=lambda path: str(path).casefold(),
            )
            if not local_files:
                st.info("No `.mf4` or `.dat` files were found. Target-only plotting is still available.")
...
tab_speed, tab_opd = st.tabs(["Speed vs Acceleration", "One-Pedal (OPD) Analysis"])
